...
<?php
	$main_table = '' . DB_PREFIX . 'parser_division_exclusion';
	...
	if(isset($_GET['find'])){
		$find = $_GET['find'];
		$where_find = 'WHERE category_name LIKE "%'.$find.'%" OR division_name_ex LIKE "%'.$find.'%" ';
	}
    ...
    //echo $sql;            
	$r = $mysqli->query($sql) or die('7 Ошибка в выборке категорий' . mysqli_error($mysqli));
	$Magazines = array();	
	if($r->num_rows > 0){
    ...
    //echo $sql;            
	$r = $mysqli->query($sql) or die('8 Ошибка в выборке магазинов' . mysqli_error($mysqli));
	$Categories = array();	
	if($r->num_rows > 0){
		while($tmp = $r->fetch_assoc()){
	...
	//Получим атрибуты	
	$sql = 'SELECT * FROM '.$main_table.' '.$where_find.' ORDER BY category_name ASC, division_name_ex ASC LIMIT '.(($page-1)*$step).', '.$step.';';
	//echo $sql;            
...
?>
	...
	<table class="attributes_table">
		...
		<tr id="new">
			...
			<td align="left"><input type="text"   id="division_name_ex" style="width:150px;" value='' placeholder="Название разделителя"></td>
		...
		<?php while($tmp = $rm->fetch_assoc()){ ?>
			<tr id="<?php echo $tmp['id']; ?>">
				...
				<td align="left"><input type="text" class="edit" id="division_name_ex<?php echo $tmp['id']; ?>"  style="width:150px;" value='<?php echo $tmp['division_name_ex']; ?>'></td>
    ...
    $(document).on('change', '.edit', function(){
		...
		var table = '<?php echo $main_table; ?>';
		   
        var magazin_id = $('#magazin_id'+target).val();
        var category_id = $('#category_id'+target).val();
        var category_name = $('#category_name'+target).val();
        var division_name_ex = $('#division_name_ex'+target).val();
     	
		$.ajax({
            type: "GET",
            url: "setup/ajax/ajax_save_category.php",
            dataType: "text",
            data: "id="+target+"&table="+table+"&magazin_id="+magazin_id+"&category_id="+category_id+"&category_name="+category_name+"&division_name_ex="+division_name_ex+"&key=edit",
            beforeSend: function(){
            },
    ...
    $(document).on('click', '#add', function(){
        ...
        var division_name_ex = $('#division_name_ex').val();
     
		$.ajax({
			type: "GET",
			url: "setup/ajax/ajax_save_category.php",
			dataType: "text",
		    data: "table="+table+"&magazin_id="+magazin_id+"&category_id="+category_id+"&category_name="+category_name+"&division_name_ex="+division_name_ex+"&key=add",
			beforeSend: function(){
			},
			success: function(msg){
				location.reload();
