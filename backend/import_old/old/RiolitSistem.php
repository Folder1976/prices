			...
			if($mymenuid && $mybrendid){
				if(!queryresult("select id from products where name='$name'",'id')){
					...
					if($_POST['addautomate']){
						...
						mysql_query("insert into product_prices(name,productid,status,magazinid,garant,price,dater) 
						values('".ltrim(rtrim($list[$i][3]))."','$pid','1','1','".ltrim(rtrim($list[$i][5]))."','".ltrim(rtrim($list[$i][4]))."','$dater')") or die(mysql_error());
					}
				}
			}else{
				echo '- - '.$mymenuid.' - '.$mybrendid.' - Товар: '.$name.'<br>';
